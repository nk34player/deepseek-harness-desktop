...
const signedIf = "${{ secrets.MAC_CERT_P12 != '' && secrets.MAC_CERT_PASSWORD != '' }}"
const unsignedIf = "${{ secrets.MAC_CERT_P12 == '' || secrets.MAC_CERT_PASSWORD == '' }}"
...
describe('Desktop release workflow', () => {
  it('builds the macOS installer unsigned when the certificate secrets are absent', () => {
    ...
    const steps = workflowJob(workflow, 'build-macos').steps
    ...
    expect(stepByName('Import signing certificate')?.if).toBe(signedIf)
