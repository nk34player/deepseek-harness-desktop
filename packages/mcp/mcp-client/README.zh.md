...
| `useSessionWorkspace` | stdio | 否 | 改用当前打开的工作区作为 spawn 目录（而非 `cwd`），绑定到看到的第一个工作区（默认 `false`） |
...
- 设置 `useSessionWorkspace: true` 时，stdio 子进程会在当前打开的工作区（最近打开的会话的 cwd）中 spawn，而非 `cwd`（尚无会话时回退到 `cwd`／Host 工作目录）。子进程会绑定到它观察到的第一个工作区并一直留在那里：在另一个工作区的会话之间打开或切换永远不会重新定位它，因此文件索引器的搜索根保持稳定。适用于从进程 cwd 派生搜索根（例如文件索引器）的服务器。未挂载会话服务的部署会一直使用配置的 `cwd`。
...
| `ctx.sessions`（可选） | 为 `useSessionWorkspace` 跟踪当前打开的工作区；子进程绑定到看到的第一个工作区，没有出现任何工作区时回退到 `cwd` |
...
- **`useSessionWorkspace` 服务器绑定到单一工作区**：绑定到它观察到的第一个工作区。一个共享的服务器无法服务多个工作区的会话，因此它停留在第一个工作区，并忽略之后的工作区切换。
