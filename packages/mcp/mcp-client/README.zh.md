...
| `useSessionWorkspace` | stdio | 否 | 每个存活会话各启动一个服务器，分别以该会话的工作区作为 spawn 目录（而非 `cwd`）（默认 `false`） |
...
- 设置 `useSessionWorkspace: true` 时，每个存活会话都会得到自己的 stdio 子进程，以该会话的工作区（`header.cwd`）作为根目录，并且该子进程的工具会限定注册到该会话的 agent 作用域——因此像 fff 这样的文件索引器始终在它所使用的会话中搜索，绝不会返回另一个工作区的结果。会话创建时启动其子进程，会话销毁时将其拆除。适用于从进程 cwd 派生搜索根（例如文件索引器）的服务器；避免用于长驻的 GUI 桥接服务，否则每个会话都会运行一个实例。此功能需要 `agents` 服务；缺少时服务器回退到配置的 `cwd`。
...
| `ctx.agents`（按会话） | 为每个存活会话驱动一个 `useSessionWorkspace` 子进程，以该会话的工作区为根目录 |
...
- **`useSessionWorkspace` 服务器每个会话运行一次**：每个存活会话都会启动自己的子进程，以该会话的工作区为根目录，因此同工作区的会话会运行冗余的子进程。按工作区去重到单个子进程暂缓实现。
