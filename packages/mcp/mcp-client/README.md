...
| `useSessionWorkspace` | stdio | no | Spawn in the currently-open workspace instead of `cwd`, binding to the first workspace seen (default `false`) |
...
- With `useSessionWorkspace: true`, the stdio child is spawned in the currently-open workspace — the cwd of the most recently opened session — instead of `cwd` (falling back to `cwd`/host when no session is open yet). The child binds to the first workspace it observes and then stays there: opening or switching to a session in another workspace never re-points it, so a file-indexer's search root stays stable. Use this for servers that derive a search root from their process cwd (e.g. a file indexer). A server with no sessions mounted stays on its configured `cwd`.
...
| `ctx.sessions` (optional) | Track the currently-open workspace for `useSessionWorkspace`; the child binds to the first workspace seen and falls back to `cwd` when none appears |
...
- **A `useSessionWorkspace` server is bound to a single workspace** — the first it observes. One shared server cannot serve sessions in multiple workspaces, so it stays in that first workspace and ignores later session switches.
