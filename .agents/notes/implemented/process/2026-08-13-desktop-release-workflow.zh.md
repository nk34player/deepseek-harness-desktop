...
- **macOS**（`macos-14`）：`dist:mac` 暂存自包含运行时并构建 arm64 的 `.dmg`（证书机密缺失时回退到 `dist:mac:unsigned`）。
...
electron-builder.yml 设置了带架构后缀的 `artifactName`，让发布页上的 macOS 与 Windows 安装包一目了然。它还钉死了 macOS 的 `identity`（不带证书类型前缀）并启用 `notarize: true`，同时设 `npmRebuild: false`（壳进程内没有原生依赖）；工作流把 `.p12` 导入临时钥匙串，并借助仓库 secret 里的 App Store Connect API key 通过 `notarytool` 公证。当任一证书机密缺失时，导入步骤被跳过，`dist:mac:unsigned` 会置空 identity 并关闭公证（`--config.mac.identity=null --config.mac.notarize=false`），从而发布包以未签名方式产出，而不是因 identity 缺失而构建失败。
...
macOS 安装包在证书机密存在时签名并公证，缺失时则为未签名；Windows 安装包在加入 Authenticode 证书之前仍为未签名，自动更新源亦被延后。
...
- **macOS 仅在证书可用时签名并公证。** 证书缺失时发布包以未签名 dmg 产出；在加入 Authenticode 证书之前，Windows 用户仍会看到 SmartScreen 警告；两个平台都完成签名也是上线自动更新源的前置条件。
